...
export class astro {
  ...
  static moonEciFromTimeNew(startDate = new Date(1992, 3, 12)) {
    ...
    let long =
      (math.atan2(
        sinD(coordinates.long) * cosD(epsilon) -
          tanD(coordinates.lat) * sinD(epsilon),
        cosD(coordinates.long)
      ) *
        180) /
      Math.PI;
    ...
    let lat =
      (math.asin(
        sinD(coordinates.lat) * cosD(epsilon) +
          cosD(coordinates.lat) * sinD(epsilon) * sinD(coordinates.long)
      ) *
        180) /
      Math.PI;
    return [cosD(long) * cosD(lat), sinD(long) * cosD(lat), sinD(lat)].map(
      (s) => s * coordinates.r
    );
  }
  static moonPhase(date = new Date(), eciOrigin = [0, 0, 0]) {
    ...
    let cosPsi =
      Math.sin(sunPosition.declination) * Math.sin(moonPosition.declination) +
      Math.cos(sunPosition.declination) *
        Math.cos(moonPosition.declination) *
        Math.cos(sunPosition.rightAscension - moonPosition.rightAscension);
    let cosI = -cosPsi;
    let k = (1 + cosI) / 2;
    return k;
  }
  static moonPhaseFromAngle(
    viewPosition = [0, 0, 0],
    moonPosition = [1, 0, 0],
    sunPosition = [-1, 1, 0]
  ) {
    ...
    let cosPsi =
      math.dot(moonPosition, sunPosition) /
      math.norm(moonPosition) /
      math.norm(sunPosition);
  ...
  static moonPhaseFromEci(moonEci, sunEci) {
    ...
    let tanXi =
      (Math.cos(sunPosition.declination) *
        Math.sin(sunPosition.rightAscension - moonPosition.rightAscension)) /
      (Math.sin(sunPosition.declination) * Math.sin(moonPosition.declination) -
        Math.cos(sunPosition.declination) *
          Math.sin(moonPosition.declination) *
          Math.cos(sunPosition.rightAscension - moonPosition.rightAscension));
    console.log(tanXi, (math.atan(tanXi) * 180) / Math.PI);
  }
  static sunEciFromTime(date = new Date()) {
    let jdUti = astro.julianDate(
      date.getFullYear(),
      date.getMonth() + 1,
      date.getDate(),
      date.getHours(),
  ...
  static eci2latlong(
    r = [5102.508958, 6123.011401, 6378.136928],
    date = new Date(2004, 3, 6, 7, 51, 28, 386)
  ) {
    // Based on Vallado "Fundamentals of Astrodyanmics and Applications" algorithm 24, p. 228 4th edition
    // ECI to ECEF
    let jd_TT = astro.julianDate(
      date.getFullYear(),
      date.getMonth() + 1,
      date.getDate(),
      date.getHours(),
  ...
  static groundGeodeticPosition(lat = 39.586667, long = -105.64, h = 4.347667) {
    ...
    let r = math.squeeze(
      math.multiply(astro.rot(-long, 3), math.transpose([[rSigma, 0, rk]]))
    );
    let rij = math.dotDivide(
      r.slice(0, 2),
      math.norm(r.slice(0, 2)) / (rk / math.tan(lat))
    );
    // console.log(rij, r);
    return { r, vert: [...rij, r[2]] };
  }
  static rAzEl(
    r_eci = [-5505.504883, 56.44917, 3821.871726],
    date = new Date(1995, 4, 20, 3, 17, 2),
    lat = 39.007,
    long = -104.883,
    h = 2.187
  ) {
  ...
  static ecef2eci(
    r = [-1033.479383, 7901.2952754, 6380.3565958],
    date = new Date(2004, 3, 6, 7, 51, 28, 328)
  ) {
    // Based on Vallado "Fundamentals of Astrodyanmics and Applications" algorithm 24, p. 228 4th edition
    // ECI to ECEF
    let jd_UTI = astro.julianDate(
      date.getFullYear(),
      date.getMonth() + 1,
      date.getDate(),
      date.getHours(),
  ...
  static j20002Coe(state = [42164.14, 0, 0, 0, 3.0746611796284924, 0]) {
    ...
    let e = math.dotDivide(
      math.subtract(
        math.dotMultiply(vn ** 2 - mu / rn, r),
        math.dotMultiply(math.dot(r, v), v)
      ),
      mu
    );
    let en = math.norm(e);
  ...
  static coe2J2000(
    coe = { a: 42164.1401, e: 0, i: 0, raan: 0, arg: 0, tA: 0 },
    peri = false
  ) {
    ...
    let v = [[-constA * sTa], [(coe.e + cTa) * constA], [0]];
    if (peri) return [r[0], r[1], r[2], v[0], v[1], v[2]].map((s) => s[0]);
    ...
    r = math.multiply(R, r);
    v = math.multiply(R, v);
    return [r[0], r[1], r[2], v[0], v[1], v[2]].map((s) => s[0]);
  }
  ...
  static ConvEciToRic(chief_eci, deputy_eci, returnRot = false) {
    let rC = chief_eci.slice(0, 3),
      drC = chief_eci.slice(3, 6),
      rD = deputy_eci.slice(0, 3),
      drD = deputy_eci.slice(3, 6);
    ...
    let ricXd = math.dotMultiply(
      1 / math.norm(rC),
      math.subtract(drC, math.dotMultiply(math.dot(ricX, drC), ricX))
    );
    ...
    return [
      ...
      ...math.squeeze(
        math.add(
          math.multiply(Cd, math.transpose([math.subtract(rD, rC)])),
          math.multiply(C, math.transpose([math.subtract(drD, drC)]))
        )
      ),
    ];
  }
  static Eci2Ric(chief, deputy) {
    let rC = chief.slice(0, 3),
      drC = chief.slice(3, 6);
    let rD = deputy.slice(0, 3),
      drD = deputy.slice(3, 6);
    let h = math.cross(rC, drC);
    let ricX = math.dotDivide(rC, math.norm(rC));
    let ricZ = math.dotDivide(h, math.norm(h));
    let ricY = math.cross(ricZ, ricX);
  ...
  static pointRadialDistance(lat1 = 0, lon1 = 0, bearing = 10, rdistance = 5) {
    ...
    if (Math.cos(rlat) == 0 || Math.abs(Math.cos(rlat)) < 0.000001) {
      ...
      rlon =
        rlon1 +
        Math.atan2(
          Math.sin(rbearing) * Math.sin(rdistance) * Math.cos(rlat1),
  ...
  static solveKeplersEquation(meanAnom, e) {
    ...
    while (Math.abs(del) > 1e-6) {
      del =
        (eccAnom - e * Math.sin(eccAnom) - meanAnom) /
        (1 - e * Math.cos(eccAnom));
      eccAnom -= del;
    }
    return eccAnom;
  }
  static toStkDateFormat(date) {
    ...
    time =
      time.split('GMT')[0].substring(4, time.split('GMT')[0].length - 1) +
      '.000';
    time = time.split(' ');
...
class Propagator {
  ...
  highPrecisionProp(
    ...
    // console.time('3B')
    if (this.thirdBody) {
      a = math.add(this.thirdBodyEffects(position, date), a);
    }
    // console.timeEnd('3B')

    // console.time('atm')
    if (this.atmDrag) {
      a = math.add(this.atmosphericDragEffect(position), a);
    }
    // console.timeEnd('atm')

    // console.time('solarRad')
    if (this.solarRad) {
      a = math.add(this.solarRadiationPressure(position, date), a);
    }
  ...
  recursiveGeoPotential(state = [42164, 0, 0], date) {
    ...
    for (let order = 2; order <= this.order; order++) {
      ...
      for (let m = 0; m <= order; m++) {
        if (m === 0)
          pRow.push(
            ((2 * order - 1) * sinLat * pMat[order - 1][0] -
              (order - 1) * pMat[order - 2][0]) /
              order
          );
        else if (m === order)
          pRow.push((2 * order - 1) * cosLat * pMat[order - 1][order - 1]);
        else {
        ...
        let pMpl1;
        if (m + 1 === order) {
          pMpl1 = (2 * order - 1) * cosLat * pMat[order - 1][order - 1];
        } else {
        ...
        du_dr +=
          ...
          (order + 1) *
          pM *
          (this.c[order][m] * cosArray[m] + this.s[order][m] * sinArray[m]);

        du_dlat +=
          rArray[order] *
          (pMpl1 - mTanArray[m] * pM) *
          (this.c[order][m] * cosArray[m] + this.s[order][m] * sinArray[m]);
        ...
        du_dlong +=
          ...
          m *
          pM *
          (this.s[order][m] * cosArray[m] - this.c[order][m] * sinArray[m]);
      }
      pMat.push(pRow);
    }
    ...
    let a_i =
      ((1 / r) * du_dr - (z / r / r / Math.sqrt(x * x + y * y)) * du_dlat) * x -
      (1 / (x * x + y * y)) * du_dlong * y;
    let a_j =
      ((1 / r) * du_dr - (z / r / r / Math.sqrt(x * x + y * y)) * du_dlat) * y +
      (1 / (x * x + y * y)) * du_dlong * x;
    let a_k =
      (1 / r) * du_dr * z + (Math.sqrt(x * x + y * y) / r / r) * du_dlat;

    return {
      pMat,
      a: math.squeeze(math.multiply(rot, math.transpose([[a_i, a_j, a_k]]))),
    };
  }
  rk4(state = [42164, 0, 0, 0, -3.07, 0], dt = 10, date = new Date()) {
    let k1 = this.highPrecisionProp(state, date);
    let k2 = this.highPrecisionProp(
      math.add(state, math.dotMultiply(dt / 2, k1)),
      new Date(date - (-dt / 2) * 1000)
    );
    let k3 = this.highPrecisionProp(
      math.add(state, math.dotMultiply(dt / 2, k2)),
      new Date(date - (-dt / 2) * 1000)
    );
    let k4 = this.highPrecisionProp(
      math.add(state, math.dotMultiply(dt / 1, k3)),
      new Date(date - -dt * 1000)
    );
    return math.squeeze(
      math.add(
        state,
        math.dotMultiply(
          dt / 6,
          math.add(k1, math.dotMultiply(2, k2), math.dotMultiply(2, k3), k4)
        )
      )
    );
  }
  ...
  atmosphericDragEffect(eciState = state2_init, options = {}) {
    ...
    let vrel = math
      .subtract(
        eciState.slice(3, 6),
        math.cross(rotEarth, eciState.slice(0, 3))
      )
      .map((s) => s * 1000);
    let vrelmag = math.norm(vrel);

    return math
      .dotMultiply((-0.5 * cd * area * rho * vrelmag) / m, vrel)
      .map((s) => s / 1000);
  }
  getAtmosphereDensity(h = 747.2119) {
    ...
    return (
      atData[dataRow][1] *
      Math.exp(-(h - atData[dataRow][0]) / atData[dataRow][2])
    );
  }
  solarRadiationPressure(
    state = state1_init,
    date = state1_init_Epoch,
    options = {}
  ) {
  ...
  propToTime(
    state = [6371, 0, 0, 0, (398600.4418 / 6371) ** 0.5, 0],
    ...
    while (Math.abs(t + h) <= Math.abs(tf)) {
      let rkResult = this.rkf45(
        state,
        new Date(date - -1000 * t),
        h,
        maxError,
        { a }
      );
  ...
  rkf45(state, time, h = 2000, epsilon = 1e-12, options = {}) {
    ...
    let k2 = math.dotMultiply(
      h,
      this.highPrecisionProp(
        math.add(state, math.dotMultiply(2 / 9, k1)),
        new Date(time - (-1000 * h * 2) / 9),
        a
      )
    );
...
class Launch {
  ...
  static findApogeeRendezvous(siteEci, targetStart, estimateTof = 7200) {
    try {
      ...
      let solution1 = solveLambertsProblem(
        siteEci,
        target1.slice(0, 3),
        tof1,
        0,
      ...
      let el =
        90 -
        (math.acos(
          math.dot(siteEci, solution1.v1) /
            math.norm(siteEci) /
            math.norm(solution1.v1)
        ) *
          180) /
          Math.PI;
      if (el < 0) {
      ...
      let solution2 = solveLambertsProblem(
        siteEci,
        target2.slice(0, 3),
        tof2,
        0,
      ...
      let solution3 = solveLambertsProblem(
        siteEci,
        target3.slice(0, 3),
        tof3,
        0,
      ...
      let dotPoly = Launch.lagrangePolyCalc(
        [tof1, tof2, tof3],
        [dot1, dot2, dot3]
      );
      ...
      tof2 -=
        (2 *
          Launch.answerPolynomial(dotPoly, tof2) *
          Launch.answerPolynomial(dDotPoly, tof2)) /
        (2 * Launch.answerPolynomial(dDotPoly, tof2) ** 2 -
          Launch.answerPolynomial(dotPoly, tof2) *
            Launch.answerPolynomial(ddDotPoly, tof2));
      console.log(
        estimateTof,
        tof2,
        dot2,
      ...
      if (dot2 < Launch.answerPolynomial(dotPoly, tof2))
        throw Error('Apogee not found exact');
      return tof2;
    } catch (error) {
      console.error(error);
      return estimateTof;
  ...
  static isSatIlluminated(satPos = [6900, 0, 0], sunPos = [6900000, 0, 0]) {
    let satSunAngle = math.acos(
      math.dot(satPos.slice(0, 3), sunPos) /
        math.norm(satPos.slice(0, 3)) /
        math.norm(sunPos)
    );
    if (satSunAngle < Math.PI / 2) return true;
    return !Launch.lineSphereIntercetionBool(
      math.subtract(satPos.slice(0, 3), sunPos),
      sunPos,
      [0, 0, 0],
      6371
    );
  }
  static lineSphereIntercetionBool(
    line = [-0.45, 0, 0.45],
    lineOrigin = [282.75, 0, 0],
    sphereOrigin = [0, 0, 0],
    sphereRadius = 200
  ) {
    line = math.dotDivide(line, math.norm(line));
    let check =
      math.dot(line, math.subtract(lineOrigin, sphereOrigin)) ** 2 -
      (math.norm(math.subtract(lineOrigin, sphereOrigin)) ** 2 -
        sphereRadius ** 2);
    return check > 0;
  }
  static calculateLaunch(
    siteEci = [6700, 0, 0],
    targetEci = [42164, 0, 0, 0, 3.014, 0],
    launchDate = new Date()
  ) {
    ...
    let vOptions = [
      solveLambertsProblem(siteEci, targetEndState.slice(0, 3), tof, 0, false)
        .v1,
      solveLambertsProblem(siteEci, targetEndState.slice(0, 3), tof, 0, true)
        .v1,
    ]
    ...
    if (vOptions.length > 0) {
      ...
      let elevationAngle =
        90 -
        (math.acos(
          math.dot(siteEci, vOptions) / math.norm(siteEci) / math.norm(vOptions)
        ) *
          180) /
          Math.PI;
      ...
      let relativeVel = math
        .subtract(endState.slice(3), targetEndState.slice(3))
        .map((s) => -s);
      let cats =
        (math.acos(
          math.dot(relativeVel, sunEci) /
            math.norm(relativeVel) /
            math.norm(sunEci)
        ) *
          180) /
        Math.PI;
